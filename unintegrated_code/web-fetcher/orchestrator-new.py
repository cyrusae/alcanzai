...
- PDFs (arXiv, DOI, local, PDF-from-URL): GROBID → synthesis → paper note
...
class PaperProcessor:
    ...
    Usage:
        ...
        processor.process("./paper.pdf")  # Local PDF
    ...
    def process(self, identifier: str, force: bool = False) -> bool:
        ...
        - Web URL: "https://example.com/article" (HTML articles or PDFs)
        - Local PDF: "/path/to/paper.pdf"
        - (Future) DOI: "10.1162/coli_a_00123"
        
        Args:
            identifier: Paper identifier (arXiv ID, URL, or file path)
        ...
        try:
            ...
            result_type = fetch_result['type']
            
            if result_type == 'paper':
                # PDF-based paper (arXiv, DOI, local, or PDF from URL)
                pdf_path = fetch_result['pdf_path']
                metadata = fetch_result['metadata']
                success = self._process_paper(pdf_path, metadata, identifier)
            
            elif result_type == 'article':
                # Web article (HTML content)
                metadata = fetch_result['metadata']
                content = fetch_result['content']
                success = self._process_article(metadata, content, identifier)
            
            else:
                raise ProcessingError(f"Unknown result type: {result_type}")
            ...
            # User-facing error for unsupported content types
            print(f"\n{'='*70}")
            print(f"✗ UNSUPPORTED SOURCE: {identifier}")
            print(f"{'='*70}")
            print(f"{e}\n")
            self.state.mark_failed(identifier, f"Unsupported source: {str(e)}")
    ...
    def process_batch(
        ...
        Args:
            identifiers: List of identifiers (mixed arXiv IDs, URLs, paths)
        ...
        Returns:
            Dictionary with results: {
                ...
                "failed": int,
                "skipped": int,
                "errors": list of (identifier, error_msg) tuples
            }
        """
        results = {
            "success": 0,
    ...
    def _fetch_paper(self, identifier: str) -> Dict[str, Any]:
        """
        Fetch paper/article based on identifier type.
        
        Routes based on:
        - arXiv ID (digits with dots, or arXiv.org URL)
        ...
        Returns:
            Dictionary with:
            - 'type': 'paper' (PDF-based) or 'article' (HTML-based)
            - 'pdf_path': Path to PDF (for papers only)
            - 'metadata': PaperMetadata (for papers) or ArticleMetadata (for articles)
            - 'content': Extracted markdown (for articles only)
        ...
        if self.web_fetcher.is_url(identifier):
            ...
            try:
                ...
                # Check if it was a PDF (indicated by empty content)
                if not content or metadata.source == 'pdf_from_web':
                    print("  → PDF from URL detected, routing to GROBID")
                    # PDF from URL - return as paper type
                    # For now, we note that pdf_path won't be available
                    # since web_fetcher saves PDFs with timestamps
                    # This is a known limitation we'll address in next iteration
                    return {
                        'type': 'paper',
                        'pdf_path': None,  # ← TODO: get saved PDF path from web_fetcher
                        'metadata': metadata
                    }
                else:
                    # HTML article
                ...
                # Re-raise unsupported source errors
                raise
        ...
        raise ProcessingError(
            ...
            f"Supported: arXiv ID (e.g., '2312.12345'), "
            f"Web URL (e.g., 'https://example.com/article'), "
    ...
    def _process_paper(
        ...
        """
        Process a PDF-based paper.
        
        Steps:
        1. Extract metadata with GROBID
        ...
        Args:
            ...
            metadata: Initial metadata (from fetcher)
            identifier: Original identifier (for state tracking)
    ...
    def _process_article(
        ...
        Steps:
        1. Generate synthesis with Claude
        2. Write Obsidian note
        3. Update processing state
        
        Args:
            metadata: Article metadata (from web_fetcher)
    ...
    def _get_source_type(self, identifier: str) -> str:
        ...
        Returns:
            Source type: "arxiv", "local", "web", etc.
        """
        if self.arxiv_fetcher.parse_arxiv_id(identifier):
            return "arxiv"
...
def process_paper(identifier: str, force: bool = False) -> bool:
    """
    Convenience function to process a single paper/article.
    
    Args:
        identifier: Paper identifier (arXiv ID, URL, or path)
