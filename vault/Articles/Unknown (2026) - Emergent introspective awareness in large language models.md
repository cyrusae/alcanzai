...
**Source:** [Web](https://www.anthropic.com/research/introspection)

> [!quote] Memorable Quote
> "The model recognized the presence of an injected thought immediately, before even mentioning the concept that was injected. This immediacy is an important distinction between our results here and previous work on activation steering."

## Quick Refresh

Researchers at Anthropic investigated whether large language models like Claude can genuinely introspect—that is, accurately report on their own internal states. Using a technique called concept injection (injecting known neural activity patterns into the model and asking if it notices), they found that Claude Opus 4 and 4.1 correctly detected injected concepts about 20% of the time, and when instructed to think about a topic versus not think about it, the models showed greater neural activity for the concept in the positive case. These experiments provide direct evidence that models can, under certain conditions, access and report on their actual internal representations rather than simply making plausible-sounding answers.

## Why You Cared

You care about AI transparency and interpretability because understanding what models are actually doing internally matters for debugging, improving safety, and building trustworthy systems. This paper directly addresses a core question: can we ever trust an AI's explanation of its own reasoning, or are they just sophisticated confabulators? The experimental design—comparing self-reports against ground-truth neural activity—is methodologically clever and moves beyond previous work that only showed suggestive evidence of introspection. If introspection becomes reliable, it could fundamentally change how we audit and understand AI systems.

## Key Concepts

`#introspection` `#concept-injection` `#activation-steering` `#neural-representation` `#internal-states` `#interpretability` `#large-language-models` `#anomaly-detection` `#machine-learning`
