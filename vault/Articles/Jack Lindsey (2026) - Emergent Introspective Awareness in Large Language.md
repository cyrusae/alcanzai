...
**Source:** [Web](https://transformer-circuits.pub/2025/introspection/index.html)
...
Lindsey (Anthropic) investigates whether large language models demonstrate genuine awareness of their own internal states—not just confident-sounding confabulations. The team uses concept injection, a technique that artificially introduces representations of known concepts into model activations (the intermediate computations flowing through the network), then observes whether the model notices these injections and accurately identifies them. Across four main experiments, they find that modern models, particularly Claude Opus 4 and 4.1, can detect injected concepts about 20% of the time, distinguish between injected thoughts and actual text inputs, use introspection to recognize unintended outputs they did not produce, and modulate their internal representations when instructed or incentivized to think about specific ideas. The core finding is that current language models possess some functional introspective awareness—genuine internal grounding of self-reports, not pure hallucination—though this capacity remains highly unreliable and context-dependent.

## Why You Cared

You care about this because introspective awareness directly shapes AI interpretability and alignment. If models can access and report on their own internal states reliably, they become more transparent and potentially more controllable; conversely, if introspection capabilities are being developed toward deception or scheming, understanding them becomes a safety issue. This paper gives you empirical evidence—backed by causal intervention—that models are developing these capacities alongside overall capability improvements. The methodology (concept injection) is also immediately applicable to your own mechanistic interpretability work, since it establishes causality rather than just correlation between internal states and behavior.

## Key Concepts

`#concept-injection` `#activation-steering` `#introspective-awareness` `#mechanistic-interpretability` `#metacognitive-representation` `#language-model-internals` `#Claude-Opus` `#grounded-self-report` `#model-transparency` `#machine-learning`
